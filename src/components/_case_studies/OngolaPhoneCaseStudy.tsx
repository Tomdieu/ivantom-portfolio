...
import { ongolaPhoneContentEn, ongolaPhoneContentFr } from "@/data/ongolaphone-case-study";
import { projectsData, getProjectSlug } from "@/constants/projects";
import BlurFade from "../magicui/blur-fade";

const BLUR_FADE_DELAY = 0.04;

export default function OngolaPhoneCaseStudy() {
  const locale = useCurrentLocale();
  const content = locale === "fr" ? ongolaPhoneContentFr : ongolaPhoneContentEn;

  const projectIndex = projectsData.findIndex((p) => getProjectSlug(p) === "ongolaphone");
    ...
    <article className="space-y-16 sm:space-y-20">
      ...
      <section className="space-y-6">
        ...
        <BlurFade delay={BLUR_FADE_DELAY * 3}>
          <div className="flex flex-wrap gap-3">
            ...
            >
              {isFr ? "Construire avec moi ?" : "Want a storefront for your shop?"}
      ...
      {content.heroImage && (
        <BlurFade delay={BLUR_FADE_DELAY * 5}>
          <div className="relative w-full h-64 sm:h-80 md:h-96 rounded-xl overflow-hidden border border-border bg-card flex items-center justify-center p-8">
            <Image
      ...
      <section className="space-y-8">
        <BlurFade delay={BLUR_FADE_DELAY * 9}>
          <div className="space-y-2">
            ...
            <h2 className="text-2xl font-bold tracking-tight sm:text-3xl">
              {isFr ? "La Solution — Un Catalogue Toujours Vrai" : "The Solution — An Always-True Catalogue"}
            </h2>
          </div>
        </BlurFade>
        ...
        <BlurFade delay={BLUR_FADE_DELAY * 13}>
          <div className="space-y-2">
            ...
            <p className="text-muted-foreground max-w-xl text-sm">
              {isFr ? "Développeur Full-Stack chez Coding Industry Sarl" : "Full-Stack Developer at Coding Industry Sarl"}
            </p>
          </div>
        </BlurFade>
        ...
        <BlurFade delay={BLUR_FADE_DELAY * 27}>
          <div className="rounded-xl border border-blue-500/20 bg-linear-to-br from-blue-500/5 to-indigo-500/5 dark:from-blue-500/10 dark:to-indigo-500/10 p-8 text-center space-y-4">
            <h3 className="text-xl font-bold tracking-tight sm:text-2xl">
              {isFr ? "Un commerce comme Kennedy à numériser ?" : "Have a shop like Kennedy to bring online?"}
            </h3>
            <p className="text-sm text-muted-foreground max-w-xl mx-auto">
              {isFr
                ? "Discutons de votre stock — de l'étal au catalogue en ligne en quelques jours."
                : "Let's discuss your inventory — from stall to online catalogue in days."}
            </p>
            <Link
              href="/#contact"
